...
from .analytical_model import AnalyticalSolution
...
def plot_mode_shapes(
    ...
    fig : Figure, optional
        Figure on which to overlay the mode shapes. If None, a new figure
        is created.
    ...
    >>> visualisation.plot_mode_shapes(sol_analytical)
    """
    if modes is None:
    ...
    if fig is None:
        ...
        for ax, k in zip(axes, modes):
            ax.plot(sol.x, sol.Phi[:, k], color="tab:pink", linewidth=2)
            ax.set_xlabel("$x$ [m]")
        ...
        for ax, k in zip(axes, modes):
            ...
            ax.plot(sol.x, sol.Phi[:, k], color="tab:pink", linewidth=2, label="analytical")
            ax.legend()
